...
// route for HOD to see or view their allocated Unit in the control Panel
router.get('/view-particular-hod-assigned-units', verifyToken, async (req, res) => {
  const hodId = req.user._id
  const units = await Unit.find({ HODId: hodId })
    .populate('Subject') // Populate the Subject field as is
    .populate({
      path: 'FacultyId', // Populate the FacultyId field
      select: 'fullName email mobile profilePhoto', // Specify the fields to include
    })
  if (units) {
    res.json({ success: true, message: "Units Retrived Successfully", units })
  } else {
    res.json({ success: false, message: "Error while fetching units" })
  }
})
